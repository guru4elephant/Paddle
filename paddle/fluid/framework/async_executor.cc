...
void AsyncExecutor::StopServer() { _fleet_ptr->stop_server(); }

void AsyncExecutor::GatherServers(const std::vector<uint64_t>& host_sign_list,
                                  int node_num) {
  _fleet_ptr->GatherServers(const_cast<uint64_t*>(host_sign_list.data()),
                            node_num);
}
...
/**
 * trainer_desc_str includes basic training information
 */
void AsyncExecutor::RunFromFile(const ProgramDesc& main_program,
  ...
  trainer->InitTrainerEnv();
  trainer->InitOtherEnv();
