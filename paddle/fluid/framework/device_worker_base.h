...
class DeviceWorkerBase {
 public:
  DeviceWorkerBase() {}
  virtual ~DeviceWorkerBase() {}
  void Initialize(const DeviceWorkerDesc& desc) = 0;
  void SetRootScope(Scope* root_scope) { root_scope_ = root_scope; }
...
class SingletonWorker : public DeviceWorkerBase {
 public:
  SingletonWorker() {}
};

class CPUWorkerBase : public DeviceWorkerBase {
 public:
  CPUWorkerBase() {}
...
class HogwildWorker : public CPUWorkerBase {
 ...
 protected:
  void CreateThreadOperators(const Program& program);
